...
//ctrl is either starts or end, both the commands use the same elements besides that    
void constructControlPacket(char * ret,char ctrl, char* fileName, size_t fileSize){//nao esquecer de alocar espaço suficiente no ret
...
/* Como se faz a numeraçao*/
//K = 256*L2 +L1 = 2⁸ * L2 + L1 = L2<<8 +L1, L2 is the most significant byte of dataSize and L1 the least signifcant byte
void constructDataPacket(char * ret, char* data, size_t dataSize, int msgNr){//nao esquecer de alocar espaço suficiente no ret
    ret[APP_CTRL_IDX] = MSG_FLAG;
